...
describe("getSessions", () => {
  ...
  it("returns only alive sessions by default", async () => {
    ...
    const liveOnly = await getSessions(config, gitCache, transcriptIndexCache);
    expect(liveOnly.map((s) => s.sessionId)).toEqual(["alive-session"]);

    const withDead = await getSessions(config, gitCache, transcriptIndexCache, { includeDead: true });
  ...
  it("returns an empty array when there are no session files", async () => {
    const sessions = await getSessions({ sessionsDir, projectsDir }, gitCache, transcriptIndexCache);
